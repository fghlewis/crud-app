...
- [x] 3. Folder tour
- [ ] 4. Set up Supabase (user does this in browser — create project, get connection string)
- [ ] 5. Create `.env.local` (explain environment variables and why secrets live here)
- [ ] 6. Write the Drizzle schema (`db/schema.ts` — notes table with id, title, body, created_at)
- [ ] 7. Generate and run the first migration (explain what a migration is, show the SQL)
- [ ] 8. Create the database client (the file that connects Next.js to Supabase)
- [ ] 9. Build the page — list view first (`app/notes/page.tsx`, server component, fetch from DB)
- [ ] 10. Add the create form (explain server actions vs. the old way)
- [ ] 11. Test end to end (create a note, check Supabase dashboard)
- [ ] 12. Add edit (inline editing, introduces `useState` / `useTransition` / `revalidatePath`)
- [ ] 13. Add delete (delete button per note)
- [ ] 14. Recap (study guide of every concept covered + 2-3 exercises to try solo)
